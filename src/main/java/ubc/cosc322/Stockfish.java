...
public class Stockfish extends BasePlayer {
    ...
    private static final double MOBILITY_WEIGHT = 0.4;
    private static final double BLOCKING_WEIGHT = 1.0;
    private static final double TERRITORY_WEIGHT = 0.3;
    ...
    */
     ...
     * It now includes a new territorial control heuristic.
     */
    private double calculateCombinedHeuristic(Map<String, Object> moveMap, LocalBoard board) {
        ...
        List<Integer> queenTarget = (List<Integer>) moveMap.get(AmazonsGameMessage.QUEEN_POS_NEXT);
        double territoryScore = computeTerritorySize(queenTarget, board);
        return blockingScore * BLOCKING_WEIGHT + mobilityScore * MOBILITY_WEIGHT + territoryScore * TERRITORY_WEIGHT;
    }
    
    /**
     * Computes the territory size using a flood-fill algorithm based on queen moves.
     * Starting from the queen's target position, it follows all eight directions (using queen move rules)
     * and counts all reachable empty cells. This assumes that board.getState() returns a 2D int array
     * where a 0 indicates an empty cell.
     */
    private int computeTerritorySize(List<Integer> queenTarget, LocalBoard board) {
        if (queenTarget == null || queenTarget.size() < 2) {
            return 0;
        }
        ...
        int[][] state = board.getState(); // assuming board state is a 2D int array
        int n = state.length;
        ...
        // Directions for queen moves (all eight directions)
        ...
        while (!queue.isEmpty()) {
            ...
            for (int d = 0; d < 8; d++) {
                int x = pos[0] + dx[d];
                int y = pos[1] + dy[d];
                // Follow the direction until we hit a non-empty cell or the board boundary
                while (x >= 0 && x < n && y >= 0 && y < n && state[x][y] == 0) {
                    String key = x + "," + y;
    ...
    private void printBestMoves(TreeNode rootNode) {
        if (!rootNode.children.isEmpty()) {
            ...
            for (int i = 0; i < showTopN; i++) {
                ...
                double territoryHeuristicValue = Math.round(computeTerritorySize(move.getQueenTarget(), child.board) * TERRITORY_WEIGHT * 100.0) / 100.0;
                double totalHeuristicValue = blockingHeuristicValue + mobilityHeuristicValue + territoryHeuristicValue;
